...
class AcousticPathReparamIntegrator : public TimeDependentIntegrator<Float, Spectrum> {
public:
    MTS_IMPORT_BASE(TimeDependentIntegrator, m_max_depth, m_rr_depth, 
                    m_max_time, m_wavelength_bins, m_time_step_count)
    MTS_IMPORT_TYPES(Scene, Sampler, Medium, Emitter, EmitterPtr, BSDF, BSDFPtr, Histogram)

    AcousticPathReparamIntegrator(const Properties &props) : Base(props) {
    ...
    std::pair<Spectrum, Mask> trace_acoustic_ray(const Scene *scene,
        ...
        if constexpr (is_cuda_array_v<Float>) {
            ...
            //std::cout << "primary_ray" << primary_ray << std::endl;
            ...
            Mask valid_ray = reduce_mask_with_primary(valid_ray_pair, arange_indices, nb_pimary_rays);

            Float band_id_res = gather<Float>(band_id, arange_indices);
            ...
            for (size_t depth = 1;; ++depth) {
                ...
                distance += si.t;
                Float distance_0 = gather<Float>(distance, arange_indices);
                Float distance_1 = gather<Float>(distance, arange_indices + nb_pimary_rays);
                Float distance_res = 0.5f * distance_0 + 0.5f * distance_1;
                ...
                // ---------------- Intersection with emitters ----------------
                {
                    ...
                    // LOG result
                    Float time_frac = (distance_res / discretizer) * hist->size().x();

                    Mask hit_emitter_reduce = reduce_mask_with_primary(hit_emitter, arange_indices, nb_pimary_rays);

                    // //std::cout << "throughput[0]: " << throughput[0] << std::endl;
                    // //std::cout << "emission_weight: " << emission_weight << std::endl;
                    // //std::cout << "hit_emitter: " << hit_emitter << std::endl;
                    // //std::cout << "hit_emitter_reduce: " << hit_emitter_reduce << std::endl;
                    // //std::cout << "time_frac: " << time_frac << std::endl;
                    // //std::cout << "band_id: " << band_id << std::endl;

                    hist->put(Point2f(time_frac, band_id_res), result, hit_emitter_reduce);

                    //std::cout << "hitemitter" << std::endl;

                    // Trace ray straight through the emitter
                    Ray3f passthru = Ray3f(si.p, ray.d, 0.f, ray.wavelengths);
                    SurfaceInteraction3f si_passthru = scene->ray_intersect(passthru, HitComputeMode::Differentiable, hit_emitter);
                    Ray3f new_ray = Ray3f(si_passthru.p, ray.d, 0.f, ray.wavelengths);
                    SurfaceInteraction3f new_si = scene->ray_intersect(new_ray, HitComputeMode::Differentiable, hit_emitter);
                ...
                //std::cout << "279:cout(active)" << count(active) << std::endl;
                ...
                for (size_t ls = 0; ls < m_dc_light_samples; ls++) {
                    ...
                    std::cout << "ray_ls: " << ray_ls << std::endl;
                ...
                std::vector<Float> contribs_ls_time_frac(m_dc_light_samples);
                ...
                for (size_t ls = 0; ls < m_dc_light_samples; ls++) {
                    ...
                    Float ds_ls_ls_dist = ds_ls[ls].dist;

                    Float expected_distance_ls = 
                        gather<Float>(ds_ls_ls_dist, arange_indices) * 0.5f +
                        gather<Float>(ds_ls_ls_dist, arange_indices + nb_pimary_rays) * 0.5f;

                    // Logging the result
                    Float time_frac = ((distance_res + expected_distance_ls) / discretizer) * hist->size().x();

                    contribs_ls[ls] = throughput * emitter_val_ls[ls] / emitter_pdf * bsdf_val * mis;
                    contribs_ls_time_frac[ls] = time_frac;

                    //std::cout << "contribs_ls_time_frac[ls] = " << time_frac << std::endl;
                ...
                if (m_dc_light_samples > 1) {
                    ...
                    for (size_t ls = 0; ls < m_dc_light_samples; ls++) {
                        ...
                        time_frac += contribs_ls_time_frac[ls];
                    }

                    contrib /= m_dc_light_samples;
                    time_frac /= m_dc_light_samples;
                    ...
                    //std::cout << "emittersampling" << std::endl;

                    //Log result
                    hist->put({ time_frac, band_id_res }, result, reduce_mask_with_primary(active_e, arange_indices, nb_pimary_rays));

                } else {
                    Throw("AcousticPathReparamIntegrator: m_dc_light_samples < 2 not implemented!");
                }
                ...
                //std::cout << "sample_main_bs.pdf" << sample_main_bs.pdf << std::endl;
                ...
                //std::cout << "si.shape" << si.shape << std::endl;
                //std::cout << "bsdf_value" << bsdf_value << std::endl;
                ...
                //std::cout << "bsdf_value: " << bsdf_value << std::endl;
                //std::cout << "bsdf_value_pdf: " << bsdf_value_pdf << std::endl;

                active &= any(neq(throughput, 0.f));

                //std::cout << "621:cout(active)" << count(active) << std::endl;
                if (none(active))
                    break;
            ...
            Throw("AcousticPathReparamIntegrator: currently this integrator must be run on the GPU.");
    ...
    std::string to_string() const override {
        return tfm::format("AcousticPathReparamIntegrator[\n"
            "  max_depth = %i,\n"
            "  rr_depth = %i\n"
...
protected:
    ...
    Mask reduce_mask_with_primary(Mask mask, UInt32 arange_indices, size_t nb_pimary_rays) const {
        return
            gather<Mask>(mask, arange_indices) ||
            gather<Mask>(mask, arange_indices + nb_pimary_rays);
    }
    ...
    Point3f estimate_discontinuity(const std::vector<RayDifferential3f> &rays,
        ...
        if (rays.size() < 2 || rays.size() != sis.size())
            Throw("AcousticPathReparamIntegrator::estimate_discontinuity: invalid number of samples for discontinuity estimation");
...
MTS_IMPLEMENT_CLASS_VARIANT(AcousticPathReparamIntegrator, TimeDependentIntegrator);
MTS_EXPORT_PLUGIN(AcousticPathReparamIntegrator, "Differentiable Acoustic Path Tracer integrator");
